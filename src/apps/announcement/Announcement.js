...
import { Button, Table, Image, Input, Modal, Form, Select, Col, DatePicker, TimePicker, message, Divider, Spin } from 'antd';
...
function Announcement() {
	...
	let columns = [
		...
		},
		{
			...
			render: (_, image) => (
				<>
					<Image width={200} height={100} src={image?.image?.url ? process.env.REACT_APP_API_URL + image.image.url : noImg} alt={image.image.url} />
				</>
			),
		},
		{
		...
		},
		{
			...
			key: 'post_status',
			render: (_, item) => (item.post_status === 'Scheduled' ? <span style={{ color: 'red' }}>{item.post_status}</span> : <span>{item.post_status}</span>),
		...
		},
		{
			...
			render: (_, record) => (
				<div class='flex-container'>
					...
					<Button type='link' icon={<img src={trashIcon} alt='delete' />} onClick={() => onDelete(record.id)} />
				</div>
			),
		},
						...
						axios
							.put(
								`${URLreScript}/${editValue.id}`,
								{
									...
									detail: value.detail,
									date_announced:
										value.publish_status === 'Scheduled' ? `${value.schedule_date.format('yyyy-MM-DD') + 'T' + value.schedule_time.format('HH:mm')}:00.000+07:00` : dateNow,
									date_expired: `${value.expire_date.format('yyyy-MM-DD') + 'T' + value.expire_time.format('HH:mm')}:00.000+07:00`,
									image: imageId,
								},
								headers,
			...
			>
				...
				>
					<div style={{ flex: 1 }}>
						...
						>
							<Input placeholder='Please input title' style={{ borderRadius: 20 }} />
						</Form.Item>
						<Form.Item
							label={
								<div>
						...
						>
							<div>
								...
								{pickedImage ? (
									<div>
										<img style={{ width: '100%', height: '40vh' }} src={pickedImage} alt='test' />
									</div>
								) : null}
					...
					<div style={{ flex: 1 }}>
						...
						>
							<Input.TextArea placeholder='Please input details' style={{ minHeight: '20vh' }} />
						</Form.Item>
						<Form.Item
							name='publish_status'
							label='Publish'
	...
	return (
		<>
			...
			{loading ? <Loading /> : <Table columns={columns} scroll={{ x: 1200 }} dataSource={searchName === '' ? data : data.filter((item) => item.title_name.toLowerCase().includes(searchName))} />}
			{value != null ? <EditAnnouncement visible={editVisible} editValue={value} onCancel={closeEditModal} /> : null}
			{newVisible ? (
				<CreateAnnouncement
					visible={newVisible}
					onCancel={() => {
