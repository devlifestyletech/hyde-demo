...
function Announcement () {
  ...
  let columns = [
    {
      ...
      render: (_, image) => (<>
        <Image
          width={200}
          height={100}
          src={image?.image?.url ? process.env.REACT_APP_API_URL +
            image.image.url : noImg}
          alt={image.image.url}
        />
      </>),
    }, {
      ...
      title: 'Action',
      dataIndex: 'operation',
      render: (_, record) => (<div class='flex-container'>
        <Button
          type='link'
  ...
  const fetchData = async () => {
    ...
    if (month.split('-')[1] === '12') {
      ...
      gte = month + '-01'
      lt = `${month.split('-')[0]}-${parseInt(month.split('-')[1]) + 1 < 10
        ? `0${parseInt(month.split('-')[1]) + 1}`
        : `${parseInt(month.split('-')[1]) + 1}`}-01`
    }

    let url = `${URLreScript}${month === ''
      ? '?'
      : `?date_announced_gte=${gte}&date_announced_lt=${lt}&`}_sort=createdAt:desc`
    console.log(url)
    await axios.get(url, headers).then((response) => {
      ...
      response.data.forEach((announce, index) => {
        let date_an = format(
          ...
          'dd MMM yyyy HH:mm', { timeZone: 'Asia/Bangkok' })
        let date_ex = format(
        ...
        let announceData = {
          ...
          date_expired_show: date_ex, ...announce,
        }
  ...
  const EditAnnouncement = ({ visible, editValue, onCancel }) => {
    ...
    let date_an = format(
      ...
      'yyyy-MM-dd HH:mm', { timeZone: 'Asia/Bangkok' })
    let date_ex = format(
    ...
    const handleValue = () => {
      form.setFieldsValue({
        ...
        schedule_date: editValue.post_status === 'Scheduled' ? moment(
          date_an.split(' ')[0]) : '',
        schedule_time: editValue.post_status === 'Scheduled' ? moment(
          date_an.split(' ')[1], 'HH:mm') : '',
    ...
    const handleEditChange = async (value) => {
      ...
      if (imageFile == null) {
        axios.put(`${URLreScript}/${editValue.id}`, {
          title_name: value.title_name,
          post_status: value.publish_status === 'Now'
            ? 'Published'
            : value.publish_status,
          announcer: session.user.fullname,
          detail: value.detail,
          link: value.link,
          date_announced: value.publish_status === 'Scheduled'
            ? `${value.schedule_date.format('yyyy-MM-DD') + 'T' +
            value.schedule_time.format('HH:mm')}:00.000+07:00`
            : dateNow,
          date_expired: `${value.expire_date.format('yyyy-MM-DD') + 'T' +
          value.expire_time.format('HH:mm')}:00.000+07:00`,
        }, headers).then((res) => {
          fetchData()
          closeEditModal()
  ...
  const CreateAnnouncement = ({ visible, onCancel }) => {
    ...
    const handleOnAdd = async (value) => {
      ...
      console.log('dateNowEX', `${value.expire_date.format('yyyy-MM-DD') + 'T' +
      value.expire_time.format('HH:mm')}:00.000+07:00`)
  ...
  return (<>
    ...
    {loading ? (<Loading />) : (<Table
      columns={columns}
      scroll={{ x: 1200 }}
      dataSource={searchName === '' ? data : data.filter(
        (item) => item.title_name.toLowerCase().includes(searchName))}
    />)}
