...
function ProjectProgresses() {
    const {dataBuilding,loadingBuilding,dataSize,paramsBuildingProgress} =useSelector((state) => state.BuildProgressActionRedux);
    ...
    const paramsBuilding={defaultPage: 1,
        sorter:undefined,
        filters:{
            subject:null,
        },
    ...
    //    console.log("createBuilding:",dataBuilding?.dataTop);
       await  dispatch({type:"EDIT_BUILDINGPROGRESS",payload:dataBuilding?.dataTop[0]})
    ...
    const sreachData =({ currentTarget }) => {
        if (currentTarget.value.length > 0) {
            if (  paramsBuildingProgress.filters !== undefined && paramsBuildingProgress.filters !== null) {
...
// rowSelection objects indicates the need for row selection
    ...
    const handleEdit = async ({currentTarget}) => {
        ...
        await  dispatch({type:"EDIT_BUILDINGPROGRESS",payload:edit_dataBuilding})
    ...
    const expandedRowRender = (param) => {
        ...
        const columns = [
            {title: 'Start Date', dataIndex: 'startproject', key: 'startDate'},
            {title: 'End Date', dataIndex: 'endproject', key: 'endDate'},
            {title: 'Name', dataIndex: 'name', key: 'name'},
            {title: 'Overview', dataIndex: 'persentage', key: 'overView'},
            {title: 'Detail', dataIndex: 'detail', key: 'detail'},
            {
                title: 'productImage', dataIndex: 'productImage', key: 'productImage', render: (text, record) => {
                    return (
                        <div>

                            <img width={120} height={120}
                                 src={Array.isArray(param.imageprogress) ?param.imageprogress[0].url:bill}/>
                        </div>
                    );
                },
            },
        ...
        return <Table style={{whiteSpace: 'pre'}} columns={columns} dataSource={[data]} pagination={false}/>;
    };

    const columns = [
        {title: 'No.', dataIndex: 'No', key: 'No'},
        {title: 'Project Name', dataIndex: 'subject', key: 'subject'  ,
            render: (text) => (
                <Highlighter
        ...
        },
        {
            title: 'Location', 
        dataIndex: 'Location', 
        key: 'Location',sorter: (a, b) =>a.Location.localeCompare(b.Location)
    },
        {
            title: 'Overall Progresses', 
            dataIndex: 'totalProgress', 
            key: 'totalProgress',
            sorter: (a, b) =>a.totalProgress.localeCompare(b.totalProgress)
        },
        {
            title: 'Published By', 
            dataIndex: 'creator', 
                    ...
                    <div className='flex-container'>
                        <div style={{flex: 1, display: 'flex', justifyContent: 'flex-end',}}>
                            ...
                            <Button
                                ...
                                onClick={createbuilding}> Add New Project
                            </Button>
                        </div>
                    </div>
                </div>
