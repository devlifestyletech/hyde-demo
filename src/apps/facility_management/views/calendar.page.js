...
export default function BookingCalendarPage() {
	...
	useEffect(() => {
		...
		setLoading(true);
		(async () => {
			await addressService.getAllAddresses().then((res) => setAddresses(res.data));
	...
	return (
		<>
			...
			<div className='top-container'>
				...
				<Button icon={<PlusOutlined />} type='primary' size='large' shape='round' style={{ float: 'right' }} onClick={() => setCreateModalVisible(true)}>
					Create Reservation
				</Button>
			</div>
			<div className='content-container'>
				...
				<CreateReservation visible={createModalVisible} onCancel={() => setCreateModalVisible(false)} facility={selectedFacility} time_slot={timeSlot} addresses={addresses} />
			</div>
		</>
	);
}
