...
export default function EditReservation({ data, facility, visible, onCancel }) {
  ...
  var time_slot = reservations?.map((item) => ({
    start: item?.startDateTime.toDate(),
    end: item?.endDateTime.toDate(),
  }));
      ...
      onOk() {
        ...
        let daily_stopTime =
          f?.daily_stop === 0
            ...
            : new Date(val.range[0]).setHours(f?.daily_stop, 0, 0, 0);
      ...
      <Modal
        ...
        footer={[
          <Button
            ...
            type="primary"
            onClick={() =>
              editReservationForm.validateFields().then((val) => onConfirm(val))
            }
          >
            Save
          </Button>,
