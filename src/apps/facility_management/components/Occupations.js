					...
					occupations.map((occupation, index) => (
						<div key={index} className='facilities-card'>
							<Card
								cover={
									<img
										src={occupation.image ? `${process.env.REACT_APP_API_URL}${occupation.image.url}` : noImg}
										alt='place'
										style={{
											width: '100%',
											height: 285,
							...
							>
								<Row>
									<Col span={22}>
										<div style={{ fontSize: 18, fontWeight: 'bold' }}>{occupation.room_name}</div>
									</Col>
									<Col span={2}>
										<Button
											type='link'
								...
								<div>
									{occupation.current_people < occupation.low_status_people ? (
										<Row>
											<div>
												<img src={lowIcon} alt='low' style={{ marginRight: 10 }} />
												Low
											</div>
										</Row>
									) : occupation.current_people < occupation.medium_status_people ? (
										<Row>
											<div>
												<img src={mediumIcon} alt='low' style={{ marginRight: 10 }} />
												Medium
											</div>
										</Row>
									) : (
										<Row>
											<div>
												<img src={highIcon} alt='low' style={{ marginRight: 10 }} />
												High
											</div>
										</Row>
									)}
