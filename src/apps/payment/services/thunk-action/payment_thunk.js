import {
    addressCustomer, GetBillinglist, getCountaddressCustomer,
    GetCountBillinglist, getDataPaymentDashboard,
} from '../API/payment_api'
import {
    CustomerQuery, paymentDashboardQuery, paymentQuery,
} from './payment_query'
...
const getBillingPayment = (params) => {
  return async (dispatch) => {
    ...
    if (resultParams.status) {
      ...
      count !== undefined ? dispatch(
        { type: 'CHANGE_DATA_SIZE', payload: count }) : 0
      ...
      if (data !== undefined) {
        ...
        dispatch(
          { type: 'CHANGE_STATUS_BILLING', payload: params.status })
      } else {
        dispatch({
          payload: [],
          type: 'DISPLAY_DATATABLE',
  ...
  return async (dispatch) => {
    ...
    if (resultParams.status) {
      ...
      if (data !== undefined) {
        ...
        dispatch({
          type: 'CHANGE_STATUS_BILLING',
          payload: 'Bill not generated',
        })
      } else {
