...
const updateRescrpt = async (data) => {
	let result = null;
	result = await Axios.put(`${URLreScrpit}/${data.idBilling}`, { BillsPayment_Status: 'Payment successfull' }, options)
		.then((res) => {
...
const rejectRescrpt = async (data, annotaion) => {
	...
	const result = await Axios.put(`${URLreScrpit}/${data.idBilling}`, { BillsPayment_Status: 'Wait for payment', annotation_payment: annotaion }, options)
		.then((res) => {
			return res.status === 200 ? true : false;
		})
		.catch((err) => {
...
const editAddress = async (address_id, value) => {
	const statusResult = await Axios.put(`${URLAddress}/${address_id}`, { Status_billpayment: value === undefined ? true : false }, options)
		.then((result) => {
...
const postdataRescrpt = async (data) => {
	...
	const result = await Axios.post(`${URLreScrpit}`, postData, options)
		.then(async (res) => {
...
const deleteBillingPayment = async (paymentID) => {
	...
	await Axios.get(`${URLreScrpit}/${paymentID}`, options)
		.then((result) => {
			resultDataByID = result.data.BillsPayment_Status === 'Wait for payment' ? paymentID : null;
			resultDataAddressId = result.data.BillsPayment_Status === 'Wait for payment' ? result.data.address_id : null;
		})
		.catch((err) => {
			console.error(err);
		});
...
export { GetBillinglist, GetCountBillinglist, getDataSCB, updateRescrpt, rejectRescrpt, postdataRescrpt, addressCustomer, deleteBillingPayment, getOutDate, getCountaddressCustomer };
