...
import { getDataSCB, deleteBillingPayment, getOutDate } from '../services/API/payment_api';
...
export const Table_payment = () => {
	const { status_billing, dataBilling, loadingTable, dataSize, paramsBilling, pageDefault } = useSelector((state) => state.PaymentActionRedux);
	...
	const handleTableChange = async (pagination, filters, sorter) => {
		...
		(paramsBilling.status = status_billing), (paramsBilling.defaultPage = pagination.current), (paramsBilling.pagesize = pagination.pageSize);
		paramsBilling.filters = {
			BillsPayment_Invoice: filters?.BillsPayment_Invoice !== undefined ? filters?.BillsPayment_Invoice : null,
			Address_Customer: filters?.Address_Customer !== undefined ? filters?.Address_Customer : null,
			Name_Customer: filters?.Name_Customer !== undefined ? filters?.Name_Customer : null,
			Total_BillsPayment: filters?.Total_BillsPayment !== undefined ? filters?.Total_BillsPayment : null,
		};
	...
	let columns = [
		{
			...
			render: (text) => (
				<Highlighter
					...
					searchWords={paramsBilling.filters.BillsPayment_Invoice !== null ? paramsBilling.filters.BillsPayment_Invoice : ['']}
					autoEscape
					textToHighlight={text ? text.toString() : ''}
				/>
			),
			sorter: (a, b) => a.BillsPayment_Invoice.localeCompare(b.BillsPayment_Invoice),
		},
		{
			title: 'Room Number',
			align: 'center',
			...
			render: (text) => (
				<Highlighter
					highlightStyle={{ backgroundColor: '#D8AA81', padding: 0 }}
					searchWords={paramsBilling.filters.Address_Customer !== null ? paramsBilling.filters.Address_Customer : ['']}
					autoEscape
					textToHighlight={text ? text.toString() : ''}
				/>
			),
		...
		},
		{
			...
			render: (text, record) => (
				<Space size='middle'>
					<a>
						{moment(record.BillsPayment_Date_Start).format('DD/MM/YYYY')} - {moment(record.BillsPayment_Date_End).format('DD/MM/YYYY')}
					</a>
				</Space>
			),
		},
...

		{
			...
			render: (text, record) => (
				<>
					<Button value={record.BillsPayment_Invoice} type='Default' shape='round' onClick={exportBillingModal}>
						Export
					</Button>
				</>
			),
	...
	const PendingReview = [
...

		{
			...
			render: (text, record) => (
				<>
					<Button value={record.BillsPayment_Invoice} type='Default' disabled={record.Receipt_Status} shape='round' onClick={approveBillingModal}>
						Verify
					</Button>
				</>
			),
	...
	const waitForPayment = [
		...
		},
		{
			...
			render: (text, record) => (
				<>
					<Button value={record.BillsPayment_Invoice} type='Default' shape='round' onClick={selectedRow}>
						View
					</Button>
				</>
			),
