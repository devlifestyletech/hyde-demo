...
import { postdataRescrpt } from '../../services/API/payment_api';
...
const ModalCreateBill = () => {
	...
	const { status, dataCreateBilling, paramsBilling } = useSelector((state) => state.PaymentActionRedux);
	...
	return (
		<div>
			<Modal title={'Create Invoice Payment'} visible={status} onOk={onFinish} confirmLoading={confirmLoading} onCancel={handleCancel} width={'30%'}>
				<>
							...
							<div className='row'>
								...
								<div className='col-sm'>
									<div>
										...
										>
											<RangePicker value={hackValue || value} disabledDate={disabledDate} onCalendarChange={(val) => setDates(val)} onOpenChange={onOpenChange} />
										</Form.Item>
									</div>
								</div>
							</div>
											...
											<>
												{fields.map(({ key, name, fieldKey, ...restField }) => (
													<Space key={key} style={{ display: 'flex', marginBottom: 8 }} align='baseline'>
														...
														<Form.Item
															...
															rules={[
																...
																({ getFieldValue }) => ({
																	validator(rule, value) {
																		const rexp = /^[0-9]*[.0-9]+$/;
																		if (isNaN(value) || value === undefined || !rexp.test(value)) {
																			return Promise.reject('invalid float number');
																		}
														...
														>
															<Input style={{ borderRadius: '25px' }} placeholder='Please input amount' />
														</Form.Item>
												...
												<Form.Item>
													<Button style={{ borderRadius: '25px' }} onClick={() => add()} block>
														Add{errors}
