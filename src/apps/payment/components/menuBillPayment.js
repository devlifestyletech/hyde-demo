...
export const MenuBillPayment = () => {
	const { count, paramsBilling } = useSelector((state) => state.PaymentActionRedux);
	...
	return (
		<div style={{ paddingBottom: '5vh' }}>
			<Menu mode='horizontal' onClick={selectedMenuBillPayment} selectedKeys={[`${count}`]}>
				{MenuPayment.map((e, i) => {
					return (
						<Menu.Item key={i} title={e.title}>
							<text style={{ fontWeight: 'bold', fontSize: '1rem' }}>{e.title || e.titleText}</text>
						</Menu.Item>
					);
				})}
			</Menu>
