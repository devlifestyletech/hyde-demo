...
import { Tabs, Button, Table, Image, Input, Modal, AutoComplete, Form, Select, Col, Row, Divider, Spin, message } from 'antd';
...
function NearbyService() {
	...
	const types = ['All Service', 'Shop', 'Plumber', 'Electronics', 'Moving', 'Laundry', 'Cafe', 'Restaurant'];
	const { TabPane } = Tabs;
	const { Search } = Input;
	const { Option } = Select;
		...
		},
		{
			...
			render: (_, record) => (
				<div class='flex-container'>
					<Button type='link' icon={<img src={editIcon} alt='Edit' />} onClick={() => handleEdit(record)} />
					<Divider type='vertical' style={{ height: 30 }} />
					<Button type='link' icon={<img src={trashIcon} alt='delete' />} onClick={() => onDelete(record.key)} />
				</div>
			),
		},
	];
			...
			.then((response) => {
				...
				response.data.map((item, idx) => {
					...
					return originData.push({
						...
						number: idx + 1,
						picture: item['place_image'] ? `${process.env.REACT_APP_API_URL}${item['place_image']['url']}` : noImg,
	...
	const EditService = ({ visible, editValue, onCancel }) => {
		...
		const [latitude, setLatitude] = useState(parseFloat(editValue.key.split(',')[1]));
		const [longitude, setLongitude] = useState(parseFloat(editValue.key.split(',')[2]));
		...
		const MapWithAMarker = withScriptjs(
			withGoogleMap((props) => (
				<GoogleMap defaultZoom={18} defaultCenter={{ lat: latitude, lng: longitude }}>
					<Marker draggable={true} onDragEnd={onMarkerDragEnd} position={{ lat: latitude, lng: longitude }}>
						{address ? (
							<InfoWindow>
								<div>{address}</div>
							</InfoWindow>
				...
				>
					<div style={{ flex: 1 }}>
						...
						<Form.Item
							label={
								<div>
									<span style={{ color: '#ff4d4f', fontSize: 10, position: 'relative', bottom: 5 }}>* </span>Image
								</div>
							}
						>
							<div>
								...
								{pickedImage ? (
									<div>
										<img style={{ width: '100%', height: '20vh', borderRadius: 20 }} src={pickedImage} alt='test' />
									</div>
								) : null}
						...
						<Form.Item
							label={
								<div>
									<span style={{ color: '#ff4d4f', fontSize: 10, position: 'relative', bottom: 5 }}>* </span>Map
								</div>
							}
						>
							<SearchMap />
					...
					<div style={{ flex: 1 }}>
						...
						<div class='flex-container'>
							<Col span={11}>
								...
								>
									<Input placeholder='Please input shop name' style={{ borderRadius: 20 }} />
								</Form.Item>
							</Col>
							<Col span={11}>
								<Form.Item
								...
								>
									<Input placeholder='Please input tel.' style={{ borderRadius: 20 }} />
								</Form.Item>
							</Col>
						</div>
						<Form.Item
	...
	const Loading = () => {
		return (
			<div style={{ width: '80vw', height: '100vh', textAlign: 'center', paddingTop: 300 }}>
				<Spin size='large' />
	...
	return (
		<>
			...
			<div className='flex-container'>
				<Search placeholder='Search by name' allowClear onSearch={handleSearch} style={{ width: 300, marginBottom: 20 }} onChange={handleSearchChange} />
				<Button
					size='large'
					shape='round'
					icon={<PlusOutlined />}
			...
			<Table
				...
				dataSource={
					searchTag === 'All Service'
						...
						: data.filter((item) => item.type.includes(searchTag) && item.name.toLowerCase().includes(searchName))
				}
			/>
			{value != null ? <EditService visible={editVisible} editValue={value} onCancel={closeEditModal} /> : null}
			{newVisible ? <AddNewService visible={newVisible} onCancel={closeModal} /> : null}
		</>
	);
}
