...
import '../components/styles/Statistic.css';
...
function RoomDashboardPage() {
  ...
  const fetchData = async () => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        let room4 = data.filter(
          (room) => room.occupation.room_name === 'Swimming Pool & Kid Pool'
        );
  ...
  const countTotalInRooms = (rooms) => {
    ...
    if (rooms[0].occupation.room_name === 'Room 1') {
      ...
      setTotalRoom3(total);
    } else if (rooms[0].occupation.room_name === 'Swimming Pool & Kid Pool') {
      setTotalRoom4(total);
    } else {
  ...
  const pieData = (values) => {
    let roomName = ['Room 1', 'Room 2', 'Room 3', 'Swimming Pool & Kid Pool'];
    let data = [];
    for (let i = 1; i < 5; i++) {
      data.push({
        type: roomName[i - 1],
    ...
    <>
      ...
      <Row style={{ justifyContent: 'space-evenly' }}>
        ...
        <Col span={5} offset={1} className="owner-main">
          ...
          <div className="tag tag-4">Swimming Pool & Kid Pool</div>
        </Col>
      </Row>
