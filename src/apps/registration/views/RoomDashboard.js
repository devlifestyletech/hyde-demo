...
function RoomDashboardPage() {
	...
	const config = {
		...
		color: ['rgba(209, 186, 125, 1)', 'rgba(32, 38, 58, 1)', 'rgba(216, 170, 129, 1)'],
	};
	...
	const countTotalInRoomsByDay = (rooms) => {
		...
		rooms.forEach((room) => {
			switch (room.day) {
				case 'Monday':
					room.in_count - room.out_count >= 0 ? (total[0] += room.in_count) : (total[0] += room.out_count);
					break;
				case 'Tuesday':
					room.in_count - room.out_count >= 0 ? (total[1] += room.in_count) : (total[1] += room.out_count);
					break;
				case 'Wednesday':
					room.in_count - room.out_count >= 0 ? (total[2] += room.in_count) : (total[2] += room.out_count);
					break;
				case 'Thursday':
					room.in_count - room.out_count >= 0 ? (total[3] += room.in_count) : (total[3] += room.out_count);
					break;
				case 'Friday':
					room.in_count - room.out_count >= 0 ? (total[4] += room.in_count) : (total[4] += room.out_count);
					break;
				case 'Saturday':
					room.in_count - room.out_count >= 0 ? (total[5] += room.in_count) : (total[5] += room.out_count);
					break;
				case 'Sunday':
					room.in_count - room.out_count >= 0 ? (total[6] += room.in_count) : (total[6] += room.out_count);
					break;
	...
	return (
		<>
			...
			<Row style={{ justifyContent: 'space-between', alignItems: 'end' }}>
				<Col span={8} className='range' style={{ fontSize: 24, fontWeight: 'bold', marginTop: 20 }}>
			...
			<br />
			<Row>
				<Col xs={{ span: 24 }} xl={{ span: 24 }} xxl={{ span: 11, offset: 1 }} className='bar'>
					<div className='title-graph'>Statics of Room</div>
					<div className='graph'>
						<Column {...config} />
					</div>
				</Col>
				<Col xs={{ span: 24 }} xl={{ span: 24 }} xxl={{ span: 11, offset: 1 }} className='pie'>
					<div className='title-graph'>Percentage</div>
					<div className='graph'>
						<PieGraph data={pieData([totalRoom1, totalRoom2, totalRoom3])} colors={['rgba(209, 186, 125, 1)', 'rgba(32, 38, 58, 1)', 'rgba(216, 170, 129, 1)']} />
					</div>
				</Col>
			</Row>
		</>
