		...
		<>
			{loading ? (
				...
				<div className='zone'>
							...
							(floor, index) =>
								...
								index < maxIndex && (
									...
									>
										...
										<BuildingZone isLoading={isLoading} floorsData={floors[floor]} />
									</div>
								),
						)
					) : (
					...
					<Pagination pageSize={pageSize} current={current} total={totalLength} onChange={handleChange} className='paginationProject' style={{ textAlign: 'end', marginTop: 10 }} />
				</div>
			)}
		</>
	);
