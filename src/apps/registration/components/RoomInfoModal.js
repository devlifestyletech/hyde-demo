...
export const RoomInfoModal = ({ visible, onCancel, id, owner, inhabitant, tenant, refresh, qrCode, addressId }) => {
	const header = {
		headers: {
	...
	const [appendUserModalVisibility, setAppendUserModalVisibility] = useState(false)
						...
						<TabPane tab='Owner' key='owner'>
							{owner.length ? (
								owner.map((owner, index) => (
									<div key={'owner' + index}>
										<UsersInfo user={owner} onEvent={() => refresh()} isOwner={true} />
									</div>
								))
							) : (
						...
						<TabPane tab='Auto Parking QR Code' key='qrCode'>
							...
							>
								...
								>
									{img && qrCode ? (
										<>
											<img style={{ width: 375, height: 375 }} src={process.env.REACT_APP_API_URL + qrCode} alt='bg' />
											<Button type='link' style={{ float: 'right' }} onClick={() => setImg(false)}>
												Change Image
											</Button>
										</>
									) : (
										<>
											{pickedImage ? (
												<div>
													<img style={{ width: 375, aspectRatio: 1 }} src={pickedImage} alt='picked' />
													<Button type='link' icon={<DeleteOutlined />} onClick={() => setPickedImage(null)} style={{ width: 375, marginTop: 15 }}>
														Change image
													</Button>
												</div>
											) : (
